...
import { Transcription, TranscriptionImage } from "@src/data/TranscriptionImage"
...
export class D1TranscriptionRepository {
    ...
    async upsertAndGetUserId(userId: string): Promise<number | null> {
        try {
            await this.db.insert(schema.transcriptionUsers)
                .values({ user: userId })
                .onConflictDoUpdate({
            ...
            const userRecord = await this.db.select()
    ...
    async upsertTranscription(imageId: string, transcription: Transcription): Promise<void> {
        if (transcription.human_modified) {
            // Check if a human-modified transcription already exists for this image
            const existingHumanModified = await this.db.select()
            ...
            if (existingHumanModified && existingHumanModified.length > 0) {
                ...
                await this.db.update(schema.transcriptions)
                    .set({
                        date: transcription.date,
                        text: transcription.text,
                    })
                    .where(eq(schema.transcriptions.id, existingHumanModified[0].id));
            } else {
    ...
    async updateTranscriptionText(imageId: string, transcriptionText: string): Promise<void> {
        // Check if a human-modified transcription already exists for this image
        const existingHumanModified = await this.db.select()
            .from(schema.transcriptions)
            .where(
        ...
        if (existingHumanModified && existingHumanModified.length > 0) {
            ...
            const recentNonModified = await this.db.select()
                .from(schema.transcriptions)
                .where(
                    and(
                        eq(schema.transcriptions.imageId, imageId),
            ...
            if (recentNonModified && recentNonModified.length > 0) {
                ...
                console.warn(`No non-human-modified transcriptions found for image ID: ${imageId}`);
            }
        }
    }
}