...
import {
    constructTranscriptionUpdateRequest, finalizeSuccessfulTranscriptionUpdate,
...
export function useUpdateImage() {
    ...
    const transcribe = useMutation({
        mutationFn: ({image, request, reloadOnSuccess}: {
            image: TranscribableDocument;
            request: UpdateTranscriptionRequest,
            reloadOnSuccess: boolean
        }) => {
        ...
        onSuccess: async ([image, reloadOnSuccess]: [TranscribableDocument, boolean]) => {
            await finalizeSuccessfulTranscriptionUpdate(store, image, reloadOnSuccess);
        },
    ...
    async function updateTranscription(document: TranscribableDocument, reloadOnSuccess = false): Promise<void> {
        await updateImage(store, imageRepo, document);
        const request = await constructTranscriptionUpdateRequest(document);
        transcribe.mutate({image: document, request: request, reloadOnSuccess: reloadOnSuccess});
    }
